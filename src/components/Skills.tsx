      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className={`transition-all duration-1000 ease-in-out ${
            ...
            <div className="flex animate-scroll-right space-x-8 mb-8">
              {skills.slice(0, 6).map((skill, index) => (
                <div
                  key={`row1-${skill.name}`}
                  className={`flex-shrink-0 w-20 h-20 rounded-xl bg-gradient-to-r ${skill.color} p-4 flex items-center justify-center text-white shadow-lg`}
                  style={{
          ...
          <div className={`absolute inset-0 transition-all duration-1000 ease-in-out ${
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-4">
              {skills.map((skill, index) => (
                <div
                  key={`grid-${skill.name}`}
                  className={`group p-6 rounded-xl transition-all duration-500 transform hover:scale-105 ${
                    darkMode 
                ...
                >
                  {/* Skill Icon */}
                  <div className={`w-16 h-16 mx-auto mb-4 rounded-xl bg-gradient-to-r ${skill.color} p-3 flex items-center justify-center text-white transition-all duration-300 group-hover:scale-110`}>
                    {skill.icon}
                  </div>
                  
                  {/* Skill Name */}
                  <h3 className={`text-lg font-semibold text-center mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                    {skill.name}
                  </h3>
          ...
          <div className={`absolute bottom-4 left-1/2 transform -translate-x-1/2 transition-opacity duration-300 ${
            ...
            <p className={`text-sm font-medium ${darkMode ? 'text-gray-400' : 'text-gray-600'} text-center`}>
